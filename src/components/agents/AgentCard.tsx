...
const AgentCard: React.FC<AgentCardProps> = ({ agent, index }) => {
  ...
  return (
    <motion.div
      ...
      className="rounded-xl overflow-hidden glass-card p-6 agent-card-hover"
    >
      <div className="flex flex-col h-full">
        ...
        <Link
          to={`/agent/${agent.id}`}
          className="py-2 px-4 bg-white text-blue-600 border border-blue-200 rounded-md hover:bg-blue-50 transition-colors duration-300 text-sm text-center"
        >
          Try Demo
        </Link>
      </div>
